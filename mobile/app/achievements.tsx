...
export default function AchievementsScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      {/* Header */}
      <View className="flex-row items-center px-5 pt-2 pb-4 bg-white border-b border-gray-100">
        <TouchableOpacity
          onPress={() => router.back()}
          ...
          <View className="flex-row flex-wrap gap-2">
            {BADGES.map(badge => {
              ...
              return (
                <View
                  key={badge.id}
                  className={`items-center rounded-2xl py-3.5 px-2 ${earned ? 'bg-green-50' : 'bg-gray-50'}`}
                  style={{ width: '30.5%' }}
                >
